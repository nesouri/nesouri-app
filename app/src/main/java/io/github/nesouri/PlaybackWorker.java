...
public class PlaybackWorker implements Runnable {
	...
	private static class Item {
		...
		private Item(final int gameId, final int track) {
			this.gameId = gameId;
			this.track = track;
		}
	...
	private abstract class PlayerState implements Callable<PlayerState> {
		private volatile PlayerState next = this;

		public void cancel(final PlayerState next) {
	...
	private class ErrorState extends PlayerState {
		...
		private ErrorState(final Exception exception) {
	...
	private class PreparingState extends PlayerState {
		...
		private PreparingState(final Item item) {
			this.item = item;
		}
	...
	private class StoppedState extends PlayerState {
		@Override
		protected void process() throws Exception {
			audioTrack.pause();
			audioTrack.flush();
	...
	private class SkippingState extends PlayerState {
		public static final int NEXT = 0;
		public static final int PREVIOUS = 1;

		private final int direction;

		private SkippingState(final int direction) {
	...
	private final MediaSessionCompat.Callback callbacks = new MediaSessionCompat.Callback() {
		@Override
